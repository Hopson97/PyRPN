...
class Parser:
    ...
    def parse(self, machine, lexer, token, code):
        if token.type == lex.TOKEN_NUMBER:
            ...
            while token.type != lex.TOKEN_SEMICOLON:
                if not self.parse(machine, lexer, token, body):
                    return False
                if (token.type == lex.TOKEN_END or token.type == lex.TOKEN_ERROR):
                    print("Error: Function end never found.")
                    return False
                token = lexer.nextToken()
            machine.functions[len(machine.functions)] = body
            self.symbolTable[name] = len(machine.functions) - 1    
    ...
    def compile(self, machine, source, code):
        lexer = lex.Lexer(source)
        token = lexer.nextToken()
        while not token.type == lex.TOKEN_END:
            if not self.parse(machine, lexer, token, code):
                return
            token = lexer.nextToken()